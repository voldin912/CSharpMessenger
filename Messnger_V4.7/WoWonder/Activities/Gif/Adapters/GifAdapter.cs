        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder viewHolder, int position)
        {
            try
            {
                if (viewHolder is GifAdapterViewHolder holder)
                {
                    ...
                    {
                        Glide.With(ActivityContext?.BaseContext).Load(item.Images.PreviewGif.Url).Apply(new RequestOptions().Placeholder(Resource.Drawable.ImagePlacholder)).Into(holder.Image);
                    }
                }
            }
            catch (Exception exception)
