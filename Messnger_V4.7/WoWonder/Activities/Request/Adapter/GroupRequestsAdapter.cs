...
namespace WoWonder.Activities.Request.Adapter
{
    public class GroupRequestsAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
            ...
            try
            {
                if (viewHolder is GroupRequestsAdapterViewHolder holder)
                {
                    ...
                    if (item?.GroupTab != null)
                    {
                        ...
                        GlideImageLoader.LoadImage(ActivityContext, item.GroupTab.Avatar, holder.Image, ImageStyle.CircleCrop, ImagePlaceholders.DrawableUser);
        ...
        public IList GetPreloadItems(int p0)
        {
            try
            {
                ...
                if (item.GroupTab?.Avatar != "")
                {
                    d.Add(item.GroupTab?.Avatar);
                    return d;
                }
