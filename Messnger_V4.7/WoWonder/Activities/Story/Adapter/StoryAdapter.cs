                    ...
                    if (item != null)
                    {
                        switch (item.Stories?.Count)
                        {
                            case > 0 when item.Stories[0].Thumbnail.Contains("http"):
                                ...
                                break;
                            case > 0:
                                Glide.With(ActivityContext?.BaseContext).Load(new File(item.Stories[0].Thumbnail)).Apply(new RequestOptions().CircleCrop().Placeholder(Resource.Drawable.ImagePlacholder_circle).Error(Resource.Drawable.ImagePlacholder_circle)).Into(holder.Image);
                                break;
                        }
                        ...
                        {
                            DateTime dateTimeSeen = Methods.Time.UnixTimeStampToDateTime(int.Parse(item.Stories[0].Posted));
                            holder.TimeText.Text = dateTimeSeen.ToLongDateString() + ", " + dateTimeSeen.ToShortTimeString();
                        }
