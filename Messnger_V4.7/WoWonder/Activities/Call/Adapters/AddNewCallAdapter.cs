    ...
    public class AddNewCallAdapterViewHolder : RecyclerView.ViewHolder
    {
            ...
            , Action<AddNewCallAdapterClickEventArgs> audioCallclickListener, Action<AddNewCallAdapterClickEventArgs> videoCallclickListener) : base(itemView)
        {
            try
            {
                ...
                TxtIconVideoCall.Visibility = WoWonderTools.CheckAllowedCall(TypeCall.Video) ? ViewStates.Visible : ViewStates.Gone;
                TxtIconAudioCall.Visibility = WoWonderTools.CheckAllowedCall(TypeCall.Audio) ? ViewStates.Visible : ViewStates.Gone;
            }
            catch (Exception e)
            {
                Methods.DisplayReportResultTrack(e);
