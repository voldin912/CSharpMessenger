...
namespace WoWonder.Helpers.Controller
{
    internal static class ApiRequest
    {
                ...
                if (apiStatus == 200)
                {
                    if (respond is GetUserDataObject result)
                    {
                        ...
                        await Task.Factory.StartNew(() =>
                        {
                            ...
                            {
                                SqLiteDatabase dbDatabase = new SqLiteDatabase();
                                dbDatabase.UpdateInfoLogin_Credentials(result.UserData.UserId, result.UserData.Username, result.UserData.Name, result.UserData.Avatar);
                                ...
                                {
                                    //Insert Or Update All data Groups
                                    ListUtils.MyFollowingList = new ObservableCollection<UserDataObject>(result.Followers);
                                } 
                            }
                            catch (Exception e)
                            {
        ...
        public static void SwitchAccount(Activity context)
        {
            try
            {
