...
namespace WoWonder.SQLite
{
    public class SqLiteDatabase 
    {
        ...
        internal static readonly string PathCombine = Path.Combine(Folder, AppSettings.DatabaseName + "DB_.db");
        ...
        //Delete table but wiht Need save all account settings  
        public void DropCustom()
        {
            try
            {
                ...
                //connection.DropTable<DataTables.LoginTb>(); Need save all account 
                connection.DropTable<DataTables.MyContactsTb>(); 
                connection.DropTable<DataTables.MyProfileTb>();
                connection.DropTable<DataTables.SearchFilterTb>();
                connection.DropTable<DataTables.NearByFilterTb>();
        ...
        public void InsertOrUpdateLogin_Credentials(DataTables.LoginTb db)
        {
            try
            {
                ...
                //Off All Account 
                var result = connection.Table<DataTables.LoginTb>().ToList(); //Pending , Active
                ...
                var dataUser = connection.Table<DataTables.LoginTb>().FirstOrDefault(a => a.UserId == db.UserId);
                if (dataUser != null)
                {
                    dataUser.UserId = db.UserId;
                    dataUser.AccessToken = db.AccessToken;
                    dataUser.Cookie = db.Cookie;
                    dataUser.Username = db.Username;
                    dataUser.Password = db.Password;
                    dataUser.Status = db.Status;
                    dataUser.Lang = db.Lang;
                    dataUser.DeviceId = db.DeviceId;
                    dataUser.Email = db.Email;
                    dataUser.Avatar = db.Avatar;
        ...
        //Insert Or Update data Login
        public void UpdateInfoLogin_Credentials(string userId, string userName, string name, string avatar)
        {
            try
            {
                using var connection = OpenConnection();
                ...
                var dataUser = connection.Table<DataTables.LoginTb>().FirstOrDefault(a => a.UserId == userId);
                if (dataUser != null)
                {
                    dataUser.Avatar = avatar;
                    dataUser.FullName = name;
            ...
            {
                if (e.Message.Contains("database is locked"))
                    UpdateInfoLogin_Credentials(userId, userName, name, avatar);
                else
                    Methods.DisplayReportResultTrack(e); 
            }
        }

        //Get data Login Active
        public DataTables.LoginTb Get_data_Login_Credentials()
        {
            try
            {
                ...
                var dataUser = connection.Table<DataTables.LoginTb>().FirstOrDefault(a => a.Status == "Active"); //Pending , Active
                if (dataUser != null)
                {
                    ...
                    UserDetails.Avatar = dataUser.Avatar;
        ...
        public ObservableCollection<DataTables.LoginTb> Get_List_Account_Credentials()
        {
            try
            {
                using var connection = OpenConnection();
                ...
                ListUtils.DataUserLoginList = new ObservableCollection<DataTables.LoginTb>();
                var result = connection.Table<DataTables.LoginTb>().ToList();
                foreach (var dataUser in result)
                {
                    ListUtils.DataUserLoginList.Add(dataUser);
                }

                return ListUtils.DataUserLoginList;
            }
            catch (Exception e)
            {
                if (e.Message.Contains("database is locked"))
                    return Get_List_Account_Credentials();
                else
                {
                    Methods.DisplayReportResultTrack(e);
                    return null!;
        ...
        //Delete data Login
        public void DeleteInfoLogin_Credentials(string userId)
        {
            try
            {
                using var connection = OpenConnection();
                ...
                if (dataUser != null)
                { 
                    connection.Delete(dataUser);
                }
            }
            catch (Exception e)
            {
                if (e.Message.Contains("database is locked"))
                    DeleteInfoLogin_Credentials(userId);
                else
                    Methods.DisplayReportResultTrack(e);
            }
        }
                        ...
                        Task.Factory.StartNew(() =>
                        {
                            try
                            {
                                ...
                                //Sub Categories Products
                                    ...
                                    {
                                        var subCategories = asd.ProductsSubCategories?.SubCategoriesList
                                            ?.FirstOrDefault(a => a.Key == sub.Key).Value;
                                        if (subCategories?.Count > 0)
                                        {
                                            var cat = ListUtils.ListCategoriesProducts.FirstOrDefault(a =>
                                                a.CategoriesId == sub.Key);
                                            if (cat != null)
                                            {
                ...
                using (var connection = OpenConnection())
                {
                    ...
                    foreach (var itemChatObject in chatList)
                    {
                        ...
                        if (update != null)
                        {
                            ...
                            if (item.LastMessage.LastMessageClass != null && item.LastMessage.LastMessageClass.Seen == "0" && chk == null && item.LastMessage.LastMessageClass.FromId != UserDetails.UserId && item.Mute?.Notify == "no" && Methods.AppLifecycleObserver.AppState == "Background")
                            {
                                string id = "", name = "";
