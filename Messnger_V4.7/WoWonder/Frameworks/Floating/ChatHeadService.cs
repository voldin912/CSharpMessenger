        ...
        public override StartCommandResult OnStartCommand(Intent intent, StartCommandFlags flags, int startId)
        {
            try
            {
                if (MFloatingViewManager == null)
                {
                    ...
                    {
                        rect = (Rect)intent.GetParcelableExtra(ExtraCutoutSafeArea, Class.FromType(typeof(Rect)));
                    }
                    else
                    {
#pragma warning disable CS0618
                        rect = (Rect)intent.GetParcelableExtra(ExtraCutoutSafeArea);
#pragma warning restore CS0618
        ...
        private void IconView_Click()
        {
            try
            {
                ...
                try
                {
                    var item = ChatTabbedMainActivity.GetInstance()?.ChatTab?.LastChatTab?.MAdapter?.LastChatsList?.FirstOrDefault(a => a.LastChat?.UserId == DataUser.UserId);
                    if (item?.LastChat != null && item.LastChat?.LastMessage.LastMessageClass.ToId == UserDetails.UserId && item.LastChat?.LastMessage.LastMessageClass.FromId != UserDetails.UserId)
                    {
                        ...
                        ChatTabbedMainActivity.GetInstance()?.ChatTab?.LastChatTab?.MAdapter?.NotifyDataSetChanged();
                    }
                }
        ...
        private void CreateNotification(Context context)
        {
            try
            {
                ...
                Intent resultIntent = new Intent(context, typeof(ChatTabbedMainActivity));
