    ...
    public class AppUpdaterHelper : Java.Lang.Object, IRunnable
    {
        ...
        public static async Task LoadChatAsync(bool check = false)
        {
            try
            {
                ...
                var (apiStatus, respond) = await RequestsAsync.Message.GetChatAsync(fetch, "", "0", limit, "0", limit, "0", limit);
                ...
                else
                {
                    ...
                    if (respondList > 0)
                    {
                        ...
                        {
                            ChatTabbedMainActivity.GetInstance()?.OnReceiveResult(JsonConvert.SerializeObject(result));
                        }
                        else
                        {
